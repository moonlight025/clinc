...
from linebot.models import MessageEvent, TextMessage, TextSendMessage, TemplateSendMessage, ButtonsTemplate, MessageTemplateAction, VideoSendMessage
import configparser
import random
...
def delete_map_info(UserId):
    os.remove(UserId+'_map_info.txt')
    os.remove(UserId+'_map_info_2.txt')
...
API_KEY = 'AIzaSyBLF0sfYHttcKZa7AyaUhw7-fuqYzofaSo' # API金鑰
payload = {}
headers = {}
def getLatLng(address):
    # 傳送請求
    url = f'https://maps.googleapis.com/maps/api/geocode/json?address={address}&language=zh-TW&key={API_KEY}'
    response = requests.get(url, headers=headers, data=payload)
    # print(response.text)

    # 解析並取得經緯度
    lat, lng = response.json()['results'][0].get('geometry').get('location').values()
    return lat, lng

def getNearbyClinic(UserId, loc, keyword, replyToken):
    # keyword = '' # 搜尋關鍵字，基本上會是科別

    # 傳送請求
    url = f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={loc[0]}%2C{loc[1]}&type=doctor&rankby=distance&language=zh-TW&keyword={keyword}&key={API_KEY}"
    ...
    for i in response.json()['results'][:5]:
        name = i.get('name')
        addr = i.get('vicinity')
        # t_url = 'https://www.google.com/maps/place/?q=place_id:' + i.get('place_id')
        t_url = "https://www.google.com/maps/search/?api=1&query=" + str(loc[0]) + "," + str(loc[1]) + "&query_place_id=" + i.get('place_id')
        rating = i.get('rating')
    ...
    line_bot_api.reply_message(replyToken, TextSendMessage(text = final_tmp))
    delete_map_info(UserId)
    #     tmp_str = "➲ 診所名稱:\n" + str(name) + "\n" + "➲ 診所地址:\n" + str(addr) + "\n" + "➲ Google Maps(含導航):\n" + str(t_url) + "\n" + "➲ Google 評分:\n" + str(rating)
    #     reply_arr.append(tmp_str)
    # line_bot_api.reply_message(replyToken, reply_arr)

def getRankClinic(UserId, loc, keyword, replyToken):
    # 傳送請求
    rank_arr = []
    rank_arr2 = []
    url = f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={loc[0]}%2C{loc[1]}&type=doctor&rankby=distance&language=zh-TW&keyword={keyword}&key={API_KEY}"
    ...
    for i in response.json()['results'][:10]:
        ...
        tmp_str = "➲ 診所名稱:\n" + str(name) + "\n\n" + "➲ 診所地址:\n" + str(addr) + "\n\n" + "➲ Google Maps(含導航):\n" + str(t_url) + "\n\n" + "➲ Google 評分:\n" + str(rating)
        rank_arr.append(tmp_str)
        if rating == None:
            rating = 0
        rank_arr2.append(rating)
    
    # 排序
    for i in range(len(rank_arr2)):
        for j in range(0, len(rank_arr2) - i - 1):
            if rank_arr2[j] > rank_arr2[j + 1]:
                temp = rank_arr2[j]
                rank_arr2[j] = rank_arr2[j+1]
                rank_arr2[j+1] = temp
...
def callback():
    ...
    try:
        ...
        # map_arr = ["臺北市", "新北市", "桃園市", "臺中市", "臺南市", "高雄市", "宜蘭縣", "新竹縣", "苗栗縣", "彰化縣", "南投縣", "雲林縣", "嘉義縣", "屏東縣", "花蓮縣", "臺東縣", "澎湖縣", "基隆市", "新竹市", "嘉義市"]
        check = body.find("address")
        check2 = body.find("webhookEventId")
        check3 = body.find("replyToken")
        check4 = body.find("mode")
        check5 = body.find("userId")
        if check != -1:
            addr = body[(check + 13): (check2 - 4)]
            UserId = body[(check5 + 9): (check3 - 4)]
            replyToken = body[(check3 + 13): (check4 - 3)]
            with open(UserId+'_map_info.txt', 'w',encoding="utf_8") as f:
                f.write(str(addr))

            line_bot_api.reply_message(replyToken, TemplateSendMessage(alt_text='Buttons template', template=ButtonsTemplate(text= "請選擇診所搜尋方式", actions=[MessageTemplateAction(label= "距離(沒指定科別)", text= "距離(沒指定科別)"), MessageTemplateAction(label= "科別", text= "科別"), MessageTemplateAction(label= "最新四個問診紀錄", text= "最新四個問診紀錄")])))
...
def handle_message(event):
    ...
    if event.message.text == "是":
        ...
        line_bot_api.push_message(UserId, TextSendMessage(text = "請點選「功能選單」左側的按鈕，再點選「>」按鈕，接著點選「+」按鈕內的『位置資料』，最後跳出「位置資訊」介面後點選右上角的「分享」，謝謝。(下面為示範影片)"))
        video_message = VideoSendMessage(original_content_url='https://s5.aconvert.com/convert/p3r68-cdx67/ub7vx-2alhq.webm', preview_image_url='https://s4.aconvert.com/convert/p3r68-cdx67/a0db1-hinhg.gif')
        line_bot_api.reply_message(event.reply_token, video_message)

    elif event.message.text == "距離(沒指定科別)":
        with open(UserId+'_map_info_2.txt', 'w',encoding="utf_8") as f:
            f.write("距離(沒指定科別)")
            f.write("\n")
            f.write("None")
            
        line_bot_api.reply_message(event.reply_token, TemplateSendMessage(alt_text='Buttons template', template=ButtonsTemplate(text= "請選擇排序方式", actions=[MessageTemplateAction(label= "依距離排序", text= "依距離排序"), MessageTemplateAction(label= "依排名排序", text= "依排名排序")])))

    elif event.message.text == "科別":
        with open(UserId+'_map_info_2.txt', 'w',encoding="utf_8") as f:
            f.write("科別")
        
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text = "請輸入科別代號(前面的中文數字):\n一、心血管\n二、新陳代謝與內分泌\n三、過敏・免疫與血液疾病\n四、肝膽腸胃\n五、耳鼻喉科\n六、呼吸胸腔\n七、泌尿腎臟"))

    elif event.message.text == "依距離排序":
        map_info = []
        with open(UserId+'_map_info.txt', 'r',encoding="utf_8") as f:
            lines = f.readlines()
            map_info.append(lines[0])
        
        with open(UserId+'_map_info_2.txt', 'r',encoding="utf_8") as f:
            lines = f.readlines()
            lines[0] = lines[0].replace("\n", "")
            map_info.append(lines[0])
            map_info.append(lines[1])

        loc = getLatLng(map_info[0])
        if map_info[1] == "距離(沒指定科別)":
            keyword = ""
            getNearbyClinic(UserId, loc, keyword, event.reply_token)
        
        elif map_info[1] == "科別":
            keyword = map_info[2]
            getNearbyClinic(UserId, loc, keyword, event.reply_token)

    elif event.message.text == "依排名排序":
        map_info = []
        with open(UserId+'_map_info.txt', 'r',encoding="utf_8") as f:
            lines = f.readlines()
            map_info.append(lines[0])
        ...
        if map_info[1] == "距離(沒指定科別)":
            keyword = ""
            getRankClinic(UserId, loc, keyword, event.reply_token)

        elif map_info[1] == "科別":
            keyword = map_info[2]
            getRankClinic(UserId, loc, keyword, event.reply_token)

    elif event.message.text == "一":
        with open(UserId+'_map_info_2.txt', 'a',encoding="utf_8") as f:
            f.write("\n")
            f.write("心血管")
        
        line_bot_api.reply_message(event.reply_token, TemplateSendMessage(alt_text='Buttons template', template=ButtonsTemplate(text= "請選擇排序方式", actions=[MessageTemplateAction(label= "依距離排序", text= "依距離排序"), MessageTemplateAction(label= "依排名排序", text= "依排名排序")])))

    elif event.message.text == "二":
        with open(UserId+'_map_info_2.txt', 'a',encoding="utf_8") as f:
            f.write("\n")
            f.write("新陳代謝與內分泌")
